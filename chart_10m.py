...
# within this script for chart generation, specifically for depth-limited conditions.
...
def generate_combined_chart_depth_limited(min_wind_speed, max_wind_speed, num_wind_steps,
    ...
    for i in range(num_fetch_steps):
        for j in range(num_wind_steps):
            Hs, Ts, t_min = calculate_depth_limited(WS_grid[i, j], F_grid_m[i, j], fixed_depth_m)
            Hs_values[i, j] = Hs
            Ts_values[i, j] = Ts
    ...
    plt.subplots_adjust(left=0.05, right=0.98, top=0.90, bottom=0.10) # Adjusted top margin for longer title
